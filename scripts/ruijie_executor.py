...
class RuijieExecutor(BaseExecutor):
    """锐捷设备执行器"""
    ...
    def connect(self) -> bool:
        """建立SSH连接"""
        self.log(f"正在连接到锐捷设备 {self.host}...", "INFO")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="锐捷设备专属执行器",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
使用示例:
  python ruijie_executor.py --device 192.168.1.1 --commands "show version"
  python ruijie_executor.py --host 192.168.1.1 --username admin --password xxx --query-help "no ip nat"
  python ruijie_executor.py --device 192.168.1.1 --commands "show version" --auto-help
        """
    )
